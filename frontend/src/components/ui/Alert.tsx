...
export interface AlertProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /**
   * Callback when close button is clicked (if onClose provided, shows close button)
   */
  onClose?: () => void;
 ...
 * Displays informational messages with semantic colors and icons
 *
 * @example
...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>(
  ({ variant, title, message, onClose, children, className, ...props }, ref) => {
    const baseClasses = 'rounded-md p-4 flex gap-3';
